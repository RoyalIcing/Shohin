...
func updateCounter(message: CounterMsg, model: inout CounterModel) -> Command<CounterMsg> {
	...
	case let .setCounter(newValue):
		model.counter = min(newValue, model.maximumValue)
...
func layoutCounter(model: CounterModel, context: LayoutContext) -> [NSLayoutConstraint] {
	...
	return [
		...
		mascotChoice.topAnchor.constraint(greaterThanOrEqualToSystemSpacingBelow: counterSlider.bottomAnchor, multiplier: 1.0),
		mascot.centerXAnchor.constraint(equalTo: margins.centerXAnchor),
		mascot.topAnchor.constraint(greaterThanOrEqualToSystemSpacingBelow: mascotChoice.bottomAnchor, multiplier: 1.0),
